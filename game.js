...
function spawnEnemies() {
	var timer = setInterval(
		() => {
			...
			enemies.push(
				new Enemy((time1 - time) / 1000, x, y, radius, color, velocity),
			);
		},
		1000,
	);
...
exportGameElBtn.addEventListener(
	"click",
	() => {
		let exportAnswer = [];
		answer.forEach(
		...
		answerFromEl.value = JSON.stringify(exportAnswer);
...
submitGameElBtn.addEventListener(
	"click",
	() => {
		submitFormEl.value = "";
